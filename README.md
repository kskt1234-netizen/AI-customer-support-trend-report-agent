...
> 🟦 LLM 노드 · 🟩 순수 파이썬(결정론적) 노드 · 🔶 조건부 분기

### 1) 메인 그래프 (오케스트레이터) — 라우팅만 담당

```mermaid
flowchart TD
    U([유저 질문]) --> C["classify_intent<br/>(LLM · 구조화 출력)"]
    C --> R{조건부 라우팅<br/>route_by_intent}
    R -->|SIMPLE_CHAT| S["simple_chat 노드<br/>(LLM · 자유 답변)"]
    R -->|TREND_REPORT| T[[trend_report 서브그래프<br/>↓ 아래 그림]]
    S --> E([END])
    T --> E

    classDef llm fill:#dbeafe,stroke:#3b82f6,color:#1e3a8a;
    classDef branch fill:#fef3c7,stroke:#f59e0b,color:#92400e;
    classDef sub fill:#ede9fe,stroke:#8b5cf6,color:#5b21b6;
    class C,S llm;
    class R branch;
    class T sub;
```

### 2) trend_report 서브그래프 (워커) — 병렬 수집·연산 분리·자가 수정 루프

```mermaid
flowchart TD
    G["parallel_gather<br/>(asyncio.gather 병렬 더미 수집)"] --> CG["compute_growth<br/>⚠️ 순수 파이썬 성장률 연산"]
    CG --> D["draft_report<br/>(LLM 초안)"]
    D --> CR{"critic · 2단 게이트<br/>1단 코드: 숫자·출처 존재?<br/>2단 LLM: 맥락·톤 타당?"}
    CR -->|"불합격 & retry 한도 미만"| D
    CR -->|합격| F["finalize<br/>(Pydantic 검증)"]
    CR -->|"retry 한도 도달(3회)"| B["bail_out<br/>(무한루프 방어 탈출)"]
    F --> E([END])
    B --> E

    classDef llm fill:#dbeafe,stroke:#3b82f6,color:#1e3a8a;
    classDef code fill:#dcfce7,stroke:#22c55e,color:#166534;
    classDef branch fill:#fef3c7,stroke:#f59e0b,color:#92400e;
    class D llm;
    class G,CG,F,B code;
    class CR branch;
...
> 🔁 핵심: `critic`에서 **불합격이면 `draft_report`로 되돌아가는 루프**가 형성되고,
> `retry_count`가 3에 도달하면 `bail_out`으로 탈출해 무한 루프를 방어한다.
